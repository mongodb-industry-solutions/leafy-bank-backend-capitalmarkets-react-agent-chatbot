...
-  **Vector Search**: Semantic understanding of financial market data, news articles, and social media sentiment is achieved through [MongoDB Vector Search](https://www.mongodb.com/products/platform/atlas-vector-search?utm_campaign=devrel&utm_source=github&utm_medium=referral&utm_content=market.assistant.langgraph.react&utm_term=learning.fuel). This allows the agent to find semantically similar results across market analysis, news, and social media reports. The vector search is performed on three collections:
   ...
   -  **`reports_market_sm`** Stores social media sentiment reports with a vector field named `report_embedding` containing vector representations generated by the  [voyage-finance-2](https://blog.voyageai.com/2024/06/03/domain-specific-embeddings-finance-edition-voyage-finance-2/) model.
...
-  **MongoDB Vector Search**: For semantic search capabilities
...
3. **Tool Selection:**  
   ...
   - For portfolio-specific technical analysis → `market_analysis_reports_vector_search_tool`
   - For news sentiment about portfolio assets → `market_news_reports_vector_search_tool`
   - For social media sentiment analysis → `market_social_media_reports_vector_search_tool`
   - For current asset allocation questions → `get_portfolio_allocation_tool`
   - For year-to-date portfolio returns → `get_portfolio_ytd_return_tool`
   - For market volatility metrics → `get_vix_closing_value_tool`
...
The Market Assistant ReAct Agent leverages specialized tools to access and analyze financial data from various sources. These tools are designed specifically for portfolio management and maintain clear boundaries between portfolio-specific analysis and general financial information.
...
- **Market Analysis Reports Vector Search Tool (`market_analysis_reports_vector_search_tool`)**
  - **Purpose:** Retrieves relevant technical analysis and market insights specifically for assets in the current portfolio.
  - **Data Source:** `reports_market_analysis` collection with vector embeddings generated by the voyage-finance-2 model.
  - **Key Features:**
    - Provides market trends and momentum analysis for portfolio assets.
    - Returns technical indicators (Moving averages, etc.) for traditional assets.
    - Combines vector similarity with recency scoring for optimal results.
    - Offers overall portfolio diagnosis and asset-specific insights.
    - Should be used AFTER get_portfolio_allocation_tool for context-aware analysis.
    - Intelligently falls back to the most recent report if no semantic matches are found.

- **Market News Reports Vector Search Tool (`market_news_reports_vector_search_tool`)**
  ...
  - **Key Features:**
    ...
    - Should be used WITH market_social_media_reports_vector_search_tool for complete sentiment analysis.
    - Focuses on media coverage impact on portfolio holdings.

- **Market Social Media Reports Vector Search Tool (`market_social_media_reports_vector_search_tool`)**
  - **Purpose:** Analyzes social media sentiment from Reddit, Twitter, and other platforms for assets in the portfolio.
  - **Data Source:** `reports_market_sm` collection with vector embeddings.
  - **Key Features:**
    - Returns social media sentiment analysis for portfolio assets.
    ...
    - Should be used together with news reports for comprehensive sentiment analysis.
    - Essential for understanding broader market sentiment affecting traditional assets.

- **VIX Closing Value Tool (`get_vix_closing_value_tool`)**
...
- **Tavily Search Tool (`tavily_search_tool`)**
  ...
  - **Key Features:**
    ...
    - Covers broader market trends and developments.
...
- [MongoDB Atlas](https://www.mongodb.com/atlas/database?utm_campaign=devrel&utm_source=github&utm_medium=referral&utm_content=market.assistant.langgraph.react&utm_term=learning.fuel) for database storage, vector search, and agent state management.
...
- **MongoDB Atlas** account - [Sign up for free](https://www.mongodb.com/products/platform?utm_campaign=devrel&utm_source=github&utm_medium=referral&utm_content=market.assistant.langgraph.react&utm_term=learning.fuel) and deploy an M0 cluster
...
2. Create the following collections if they do not already exist:
   ...
   - `reports_market_sm` (for storing market social media sentiment reports)
...
1. Create the vector search index for the `reports_market_analysis`, `reports_market_news`, and `reports_market_sm` collections. You can do this using the MongoDB Atlas UI or by running the following python script located in the `backend/agent/db/` directory: `vector_search_index_creator.py`. Make sure to parametrize the script accordingly.

### Step 2: Add MongoDB User

Follow [MongoDB's guide](https://www.mongodb.com/docs/atlas/security-add-mongodb-users/?utm_campaign=devrel&utm_source=github&utm_medium=referral&utm_content=market.assistant.langgraph.react&utm_term=learning.fuel) to create a user with **readWrite** access to the `agentic_capital_markets` database.
